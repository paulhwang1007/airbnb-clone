...
function deepClone(source) {
  if (React2.isValidElement(source) || !isPlainObject(source)) {
    return source;
  }
  const output = {};
...
function deepmerge(target, source, options = {
  ...
  if (isPlainObject(target) && isPlainObject(source)) {
    Object.keys(source).forEach((key) => {
      if (React2.isValidElement(source[key])) {
...
function useMediaQueryOld(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const [match2, setMatch] = React25.useState(() => {
    if (noSsr && matchMedia) {
...
function useMediaQueryNew(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const getDefaultSnapshot = React25.useCallback(() => defaultMatches, [defaultMatches]);
  const getServerSnapshot = React25.useMemo(() => {
    if (noSsr && matchMedia) {
      return () => matchMedia(query).matches;
    }
  ...
  const [getSnapshot, subscribe] = React25.useMemo(() => {
    if (matchMedia === null) {
      return [getDefaultSnapshot, () => () => {
      }];
    }
...
@mui/private-theming/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-6JHTXJN3.js.map
